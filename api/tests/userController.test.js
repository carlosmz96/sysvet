...
describe('CONTROLADOR DE USUARIOS =>', () => {
     ...
     * 3) Si está ya creado, lo elimina
     * 4) Crea el usuario
     */
    ...
    describe('Inicio de sesión (POST -- /login)', () => {
        ...
        it('devolver un json que contenga el token de autenticación', done => {
            request(app)
                .post('/api/login')
                .set('Accept', 'application/json')
                .send({ dni: `${user.dni}`, pass: `${user.pass}`, gethash: true })
                .expect('Content-Type', /json/)
                .expect(200)
                .end((err, res) => {
                    if (err) {
    ...
    /**
     * Consulta todos los usuarios mediante un conjunto de dnis
     */
    describe('Consulta de todos los usuarios mediante un conjunto de dnis (GET -- /usuariosByDnis/:dnis)', () => {

        it('devolver un json que contenga una lista con todos los usuarios encontrados', done => {
            request(app)
                .get('/api/usuariosByDnis/11111111H,30265512L')
                .set('Accept', 'application/json')
                .set({ Authorization: `${token}` })
                .expect('Content-Type', /json/)
                .expect(200, done);
    ...
    describe('Modificación de un usuario (PUT -- /modificar-usuario/:dni)', () => {
        ...
        it('devolver un json que contenga el usuario modificado', done => {
            request(app)
                .put('/api/modificar-usuario/' + `${user.dni}`)
                .set('Accept', 'application/json')
                .set({ Authorization: `${token}` })
                .send(user)
                .expect('Content-Type', /json/)
    ...
    describe('Subida de foto de perfil de usuario (POST -- /subir-foto-perfil/:dni)', () => {
        ...
        it('devolver un json que contenga la foto subida y el usuario actualizado', done => {
            request(app)
                .post('/api/subir-foto-perfil/' + `${user.dni}`)
                .set('Accept', 'application/json')
                .set({ Authorization: `${token}` })
    ...
    describe('Eliminación de foto de perfil de usuario (POST -- /eliminar-foto-perfil/:dni)', () => {
        ...
        it('devolver un json que contenga el usuario actualizado', done => {
            request(app)
                .post('/api/eliminar-foto-perfil/' + `${user.dni}`)
                .set('Accept', 'application/json')
                .set({ Authorization: `${token}` })
                .expect('Content-Type', /json/)
                .expect(200, done);
    ...
    describe('Comprobación de contraseña (GET -- /comprobar-clave/:dni/:pass)', () => {
        ...
        it('devolver un json confirmando si es válida o no', done => {
            request(app)
                .get('/api/comprobar-clave/' + `${user.dni}` + '/' + `${user.pass}`)
                .set('Accept', 'application/json')
                .expect('Content-Type', /json/)
                .expect(200, done);
        });
    ...
    describe('Modificar la contraseña de un usuario (PUT -- /modificar-clave-usuario/:dni', () => {
        ...
        it('devolver un json con el usuario actualizado', done => {
            request(app)
                .put('/api/modificar-clave-usuario/' + `${user.dni}`)
                .set('Accept', 'application/json')
                .send({ pass: `${user.pass}` })
                .expect('Content-Type', /json/)
                .expect(200, done);
